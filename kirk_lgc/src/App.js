...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      branchClosure: false,
      inputField: [
        // object of all the necessary data to one line
        {
  ...
  branchClosure = () => {
    ...
    this.state.inputField.map((currentRow, i) => {
      ...
      if (currentRow.checkbox1 && !currentRow.firstSet) {
        rowNumbers.push(currentRow.fieldNumber, "cb1");
      } else if (currentRow.checkbox1) {
        rowNumbers2.push(currentRow.fieldNumber, "cb1");
      }
      if (currentRow.checkbox2 && !currentRow.firstSet) {
        rowNumbers.push(currentRow.fieldNumber, "cb2");
      } else if (currentRow.checkbox2) {
        rowNumbers2.push(currentRow.fieldNumber, "cb2");
      }
      if (currentRow.checkbox3 && !currentRow.firstSet) {
        rowNumbers.push(currentRow.fieldNumber, "cb3");
      } else if (currentRow.checkbox3) {
        rowNumbers2.push(currentRow.fieldNumber, "cb3");
      }
      if (currentRow.checkbox4 && !currentRow.firstSet) {
        rowNumbers.push(currentRow.fieldNumber, "cb4");
      } else if (currentRow.checkbox4) {
        rowNumbers2.push(currentRow.fieldNumber, "cb4");
      }
      newArray[i] = { ...newArray[i], firstSet: true };
      this.state.inputField.map((curRow, j) => {
        if (rowNumbers2.length !== 0) {
          newArray[j] = { ...newArray[j], firstSet: false };
        }
        return newArray;
      });
      return [rowNumbers, rowNumbers2];
    });
    console.log(rowNumbers, rowNumbers2);
    if (rowNumbers.length === 4) {
      newArray[rowNumbers[0] - 1] = {
        ...newArray[rowNumbers[0] - 1],
        checkbox1: false,
        checkbox2: false,
        checkbox3: false,
    ...
    if (rowNumbers2.length === 4) {
      newArray[rowNumbers2[0] - 1] = {
        ...newArray[rowNumbers2[0] - 1],
        checkbox1: false,
        checkbox2: false,
        checkbox3: false,
  ...
  operatorButton = () => {
    ...
    if (this.state.ruleOrder.length === 0) {
      // if it is just the single (initial) line...
      this.setState({
        inputField: [
    ...
    if (this.state.ruleOrder[lastNum] === 1 && this.state.plusButtonMain) {
      // if the last line is "single" line and plus button is true (working)... (working if stacking or splitting rule is clicked before)
      this.setState({
        inputField: [
          ...this.state.inputField,
          {
      ...
      this.state.ruleOrder[lastNum] === 2 &&
      this.state.plusButtonMain
    ) {
      // if the last line is a splitting line and '+' working (as another rule is clicked)...
      this.setState({
        inputField: [
          ...this.state.inputField,
          {
    ...
    this.state.inputField.map((field, i) => {
      ...
      if (
        (field.checkbox1 ||
          ...
          field.checkbox3 ||
          field.checkbox4) &&
        newArray[prevPrevField].type === "splitting"
      ) {
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: {
      ...
      if (
        field.type === "splitting" &&
        field.sidingStyle.gridColumnStart === undefined &&
        field.sidingStyle.gridColumn ===
          undefined /*&& field.fieldNumber === this.state.inputField.length - 1*/ &&
        (field.checkbox1 || field.checkbox3)
      ) {
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: { gridColumnStart: "1", gridColumnEnd: "3" },
        };
        newArray[fieldIndex] = {
          ...newArray[fieldIndex] /*, checkbox1: false, checkbox3: false*/,
        };
      ...
      if (
        ...
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: { gridColumnStart: "3", gridColumnEnd: "5" },
        };
        newArray[fieldIndex] = {
          ...newArray[fieldIndex] /*, checkbox2: false, checkbox4: false*/,
        };
      ...
      if (
        ...
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: { gridColumn: "1 / span 1" },
        };
      ...
      if (
        ...
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: { gridColumn: "2 / span 1" },
        };
      ...
      if (
        ...
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: { gridColumn: "3 / span 1" },
        };
      ...
      if (
        ...
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: { gridColumn: "4 / span 1" },
        };
      ...
      if (
        ...
        newArray[prevField] = {
          ...newArray[prevField],
          sidingStyle: {
            gridRowStart: `${this.state.inputField.length}`,
            gridColumn: "1 / span 2",
          },
        };
  ...
  fieldNumberAndOrangeRuleItem = (fieldNum, orangeRuIt, chckbxCnfrmd) => {
    ...
    newArray[fieldNum].checkbox1 || newArray[fieldNum].checkbox2
      ? (newArray[fieldIndex] = {
          ...newArray[fieldIndex],
          fieldNumberAndOrangeRuleItem: fieldNum + 1 + orangeRuIt,
        })
      : (newArray[fieldIndex] = {
          ...newArray[fieldIndex],
          fieldNumberAndOrangeRuleItem: fieldNum + 2 + orangeRuIt,
        });
    this.setState({ inputField: newArray });
    const lastState = this.state.inputField[this.state.inputField.length - 1]
      .orangeRuleBox;
    const lastState2 = this.state.inputField[this.state.inputField.length - 1]
      .orangeRuleBox2;
    if (chckbxCnfrmd === 1 && (lastState || lastState2)) {
      newArray[fieldNum] = {
        ...newArray[fieldNum],
        checkbox1: false,
        checkbox1Confirmed: true,
      };
      this.setState({ inputField: newArray });
    }
  ...
  orangeRuleBox = (x) => {
    ...
    if (x === 1) {
      newArray[fieldIndex] = {
        ...newArray[fieldIndex],
        orangeRuleBox: !newArray[fieldIndex].orangeRuleBox,
        orangeRuleBox2: false,
      };
    }
    if (x === 2) {
      newArray[fieldIndex] = {
        ...newArray[fieldIndex],
        orangeRuleBox2: !newArray[fieldIndex].orangeRuleBox2,
        orangeRuleBox: false,
      };
    }
    this.setState({ inputField: newArray });
  ...
  logicalOperatorButton = (symbol) => {
    // function of the logical operator buttons on the left of the page
    const fieldIndex = this.state.focusField - 1;
    let newArray = [...this.state.inputField];
    if (newArray[fieldIndex].type === "single") {
      newArray[fieldIndex] = {
        ...newArray[fieldIndex],
        fieldText1: newArray[fieldIndex].fieldText1 + symbol,
      };
      this.setState({ inputField: newArray });
      ...
      if (newArray[fieldIndex].focusInput1) {
        ...
        newArray[fieldIndex] = {
          ...newArray[fieldIndex],
          fieldText2: newArray[fieldIndex].fieldText2 + symbol,
        };
      } else if (newArray[fieldIndex].focusInput3) {
        newArray[fieldIndex] = {
          ...newArray[fieldIndex],
          fieldText3: newArray[fieldIndex].fieldText3 + symbol,
        };
      } else if (newArray[fieldIndex].focusInput4) {
        newArray[fieldIndex] = {
          ...newArray[fieldIndex],
          fieldText4: newArray[fieldIndex].fieldText4 + symbol,
        };
      }
      this.setState({ inputField: newArray });
  ...
  focusFieldNumber = (number) => {
    // function to set the active clicked field's number in state
    this.setState({ focusField: number });
    console.log("field number: ", number);
  };

  whichTextField = (fieldId) => {
    // based on which field is active it turnes on and off all the other fields...
  ...
  inputChanger = (fieldText, fieldNumber) => {
    // changes the first text input value of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
  ...
  inputChanger2 = (fieldText, fieldNumber) => {
    // changes the second text input value of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
  ...
  inputChanger3 = (fieldText, fieldNumber) => {
    // changes the third text input value of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
  ...
  inputChanger4 = (fieldText, fieldNumber) => {
    // changes the fourth text input value of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
  ...
  checkboxChanger = (fieldNumber) => {
    // toggles the first checkbox of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
    newArray[fieldIndex] = {
      ...newArray[fieldIndex],
      checkbox1: !newArray[fieldIndex].checkbox1,
    };
    this.setState({ inputField: newArray });
  };

  checkboxChanger2 = (fieldNumber) => {
    // toggles the second checkbox of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
    newArray[fieldIndex] = {
      ...newArray[fieldIndex],
      checkbox2: !newArray[fieldIndex].checkbox2,
    };
    this.setState({ inputField: newArray });
  };

  checkboxChanger3 = (fieldNumber) => {
    // toggles the third checkbox of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
    newArray[fieldIndex] = {
      ...newArray[fieldIndex],
      checkbox3: !newArray[fieldIndex].checkbox3,
    };
    this.setState({ inputField: newArray });
  };

  checkboxChanger4 = (fieldNumber) => {
    // toggles the fourth checkbox of any kind of the three types of lines
    const fieldIndex = this.state.inputField.findIndex(
      (id) => id.fieldNumber === fieldNumber
    );
    let newArray = [...this.state.inputField];
    newArray[fieldIndex] = {
      ...newArray[fieldIndex],
      checkbox4: !newArray[fieldIndex].checkbox4,
    };
    this.setState({ inputField: newArray });
  };

  expanding = () => {
    // it toggles a switch stating if a splitting or stacking line is expanded to two lines
    const fieldIndex = this.state.inputField.length - 1;
    let newArray = [...this.state.inputField];
    newArray[fieldIndex] = {
      ...newArray[fieldIndex],
      expanded: !newArray[fieldIndex].expanded,
    };
    this.setState({ inputField: newArray });
    ...
    return (
      <div className="App">
        ...
        <div className="left">
          ...
          <LogicalOperators changeData={this.logicalOperatorButton} />
        </div>
        ...
        <div className="right">
          <SplittingRule
            iterator={this.operatorButton}
            ruleId={this.orderRules}
            order={this.state.ruleOrder}
            plusBtn={this.state.plusButtonMain}
            stateData={this.state.inputField}
          />
          <StackingRule
            iterator={this.operatorButton}
          ...
          />
          <BranchClosure
            branchClosure={this.branchClosure}
            state={this.state}
          />
        </div>
      </div>
